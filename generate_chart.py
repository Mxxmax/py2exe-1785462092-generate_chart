...
def load_data(path: Path) -> tuple[list, dict[str, np.ndarray]]:
    """读取首个工作表，按“统计项目”行名提取 8 项指标。"""
...
def add_combined_trends(ax: plt.Axes, dates: list, data: dict[str, np.ndarray]) -> None:
...
def add_combined_bars(ax: plt.Axes, dates: list, data: dict[str, np.ndarray]) -> None:
...
def add_multi_axis_trends(
    ax_km: plt.Axes, dates: list, data: dict[str, np.ndarray]
) -> None:
    """一张图中用三个纵轴呈现四条真实数值趋势。"""
    ax_rate = ax_km.twinx()
    ...
    ax_km.set_ylabel("单套平均公里数", color="#657A4B", fontsize=10)
    ax_rate.set_ylabel("退库比例 / 换模率", color="#9B5364", fontsize=10)
    ax_consumption.set_ylabel("百KM模耗", color="#4062A6", fontsize=10)
...
def add_dual_axis_grouped_bars(
    ax_return: plt.Axes, dates: list, data: dict[str, np.ndarray]
) -> None:
    ...
    ax_return.set_ylabel("退库-总套数 / 异常套数", color="#5D79A2", fontsize=10)
    ax_use.set_ylabel("模具使用总枚数 / 换模数量", color="#678B72", fontsize=10)
...
def add_percentage_trends(
    ax: plt.Axes, dates: list, data: dict[str, np.ndarray]
) -> None:
    """在同一百分比坐标轴中展示退库比例与换模率。"""
    specs = [
        ("退库比例", "#B85C72", "s", "-"),
        ("换模率", "#D28B26", "^", "--"),
    ]
...
def build_chart(
    ...
    dates, data = load_data(input_path)
    ...
    add_multi_axis_trends(trend_axis, dates, data)

    bar_axis = fig.add_subplot(grid[1, 0])
    add_dual_axis_grouped_bars(bar_axis, dates, data)
    ...
    for index, (number, name, color, percentage) in enumerate(line_specs):
        ...
        # 分面版不显示 1–8 编号，指标名称作为子图标题置于图框上方。
    ...
    add_dual_axis_grouped_bars(faceted_bar_axis, dates, data)
    ...
    add_percentage_trends(percentage_axis, dates, data)
    percentage_bar_axis = percentage_fig.add_subplot(percentage_grid[1, 0])
    add_dual_axis_grouped_bars(percentage_bar_axis, dates, data)
